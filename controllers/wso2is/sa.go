...
func reconcileSva(r *Wso2IsReconciler, instance wso2v1beta1.Wso2Is, log logr.Logger, err error, ctx context.Context) (ctrl.Result, error) {
	...
	err = r.Get(ctx, types.NamespacedName{Name: variables.ServiceAccountName, Namespace: instance.Namespace}, sva)
	if err != nil {
		if errors.IsNotFound(err) {
			log.Info("ServiceAccount resource " + variables.ServiceAccountName + " not found. Creating or re-creating service account")
			...
			log.Info("Failed to get service account resource " + variables.ServiceAccountName + ". Re-running reconcile.")
			return ctrl.Result{}, err
		}
	} else {
